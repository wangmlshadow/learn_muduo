...
void TcpConnection::handleRead(Timestamp receiveTime)
{
  ...
  if (n > 0)
  {
    messageCallback_(shared_from_this(), &inputBuffer_, receiveTime);  // shared_from_this()  并非裸指针传递
  }
  else if (n == 0)
  {
    handleClose();
  ...
  if (channel_->isWriting())
  {
    ...
    if (n > 0)
    {
      ...
      if (outputBuffer_.readableBytes() == 0)// 发送缓冲区已清空
      {
        ...
        if (writeCompleteCallback_)// 回调 低水位标回调函数
        {
          loop_->queueInLoop(std::bind(writeCompleteCallback_, shared_from_this()));
        }
        if (state_ == kDisconnecting)// 发送缓冲区已清空 且连接状态是kDisconnecting 要关闭连接
        {
          shutdownInLoop();
        }
      }
